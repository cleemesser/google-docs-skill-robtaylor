...
~/.claude/skills/google-drive/gsuite drive create-folder \
  --name "Project Alpha" \
...
for DOC_ID in $(jq -r '.files[].id' search_results.json); do
  ...
  CONTENT=$(~/.claude/skills/google-docs/gsuite docs read "$DOC_ID")
  ...
  }" | ~/.claude/skills/google-docs/gsuite docs replace
done
```
...
~/.claude/skills/google-drive/gsuite drive copy \
  --file-id "TEMPLATE_ID" \
  --name "Invoice #1234 - Acme Corp"

# Step 3: Read template content
NEW_DOC_ID=$(jq -r '.id' copy_result.json)
~/.claude/skills/google-docs/gsuite docs read "$NEW_DOC_ID"
...
~/.claude/skills/google-drive/gsuite drive move \
  --file-id "$NEW_DOC_ID" \
